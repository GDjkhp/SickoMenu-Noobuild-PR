...
public:
	...
	PlayerStats(const std::string& src, int sc)
		: source(src), score(sc) {}
};
...
namespace HostTab {
	...
	static bool openStatistics = false;

	void CloseOtherGroups(Groups group) {
		openGeneral = group == Groups::General;
		openStatistics = group == Groups::Statistics;
	}
	...
	void Render() {
		if (IsHost()) {
			...
			ImGui::SameLine();
			if (TabGroup("Statistics", openStatistics)) {
				CloseOtherGroups(Groups::Statistics);
			}
			if (openGeneral) {
				GameOptions options;
				if (IsInLobby()) {
			...
			if (openStatistics) {
				...
				synchronized(Replay::replayEventMutex) {
					for (std::vector<std::unique_ptr<EventInterface>>::iterator it = State.liveReplayEvents.begin(); it != State.liveReplayEvents.end(); ++it) {
						EventInterface* evt = (*it).get();
						if (evt->getType() == EVENT_TYPES::EVENT_WALK) continue;
						if (evt->getType() == EVENT_TYPES::EVENT_KILL) {
							auto it = std::find_if(players.begin(), players.end(), [evt](const PlayerStats& p) {
								return p.source == evt->getSource().playerName;
							});
							if (it == players.end())
								players.push_back(PlayerStats(evt->getSource().playerName, 1));
							else it->score++;
						}
					}
				}
				for (const auto &player : players) {
					ImGui::Text("%s: %d", player.source.c_str(), player.score);
				}
			}
			ImGui::EndChild();
		}
